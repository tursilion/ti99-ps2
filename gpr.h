...
//
// enable this define to have LEDs on port B and serial debug on PortD
//#define DEBUGMODE

#ifndef DEBUGMODE
#define putserialchar(x)
#define flipled(x)
#define print_hexbyte(x)
#else
void flipled(char led);
void print_hexbyte(unsigned char i);
#endif

// first changes to input - low bits will now be tristate, high still appear high
// then turns off the pull ups
#define TRISTATE_TO_TI		\
	flipled(0);				\
	DDRA=0x00;				\
	PORTA=0x00;	
